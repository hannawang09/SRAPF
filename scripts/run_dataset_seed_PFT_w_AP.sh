            ...
            for shots in "${shot_values[@]}"; do
                for init in "${cls_inits[@]}"; do
                    for seed in "${seeds[@]}"; do
                        ...
                        --warmup_lr "$warmup_lr" --warmup_iter "$warmup_iter" --folder "$output_folder" --skip_stage2 \
                        --ft_topk_blks "$ft_topk_blks" --model_cfg "$model_cfg" --add_ap_stage1 --eps_stage1 "$eps_stage1" \
